...
from src.common.domain.exceptions.common import UntrackedError
...
# DATADOG WAS DISABLED BY PERFORMACE AND MANY ERRORS WERE REPORTED
...
CSRF_TRUSTED_ORIGINS = [
    'https://api.collectives.dev',
    'https://api.collectives.pro',
]
...
def before_send(event, hint):
    if 'exc_info' in hint:
        ...
        if isinstance(exc_value, (DisallowedHost, UntrackedError)):
            return None
    return event


integrations = [sentry_logging, django_integration, RedisIntegration()]
sentry_sdk.init(
    dsn=SENTRY_DSN,
